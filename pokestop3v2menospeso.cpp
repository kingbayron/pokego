...
void crearGML(vector<int>nuevasolucion,vector<int>complementosolucion ,char*a,int nodos,vector<Pokeparada> b){
	...
	if(nuevasolucion.size()!=0){
		...
		<< "w 11  h 11 type \"triangle, point down\" fill  \"#008080\"] LabelGraphics"
		<< " " << "[text " << " " << "\""
		...
		for (int i = 1; i < nuevasolucion.size(); i++){
			...
			<< "w 11  h 11 type \"triangle, point down\" fill  \"#FF6600\"] LabelGraphics"
			<< " " << "[text " << " " << "\""
...
vector<int> treeopt(vector<int> &a, vector<vector<double> > b){
	...
	for (int i = 0; i < aux.size()-2; ++i){
		for (int j = i+1; j < aux.size()-1; ++j){
			for (int k = j+1; k < aux.size(); ++k){
				vector<int> aux2=treeoptswap(aux,i,j,k,b);
				int costo1=calculaValor(aux2,b);
				if(costo1<costo){
					aux=aux2;
					costo=costo1;
...
vector<int> agregarnodoalazar(vector<int> solucion_actual,vector<vector<double> > distancias,  double presupuesto, vector<Pokeparada> todoslosnodos){
	...
	for(int i=0;i<copiasolucion.size();i++){
		...
		if(calculaValor(copiasolucion,distancias)<=presupuesto){
			...
			copiasolucion.erase(copiasolucion.begin()+i);
		}
	}
	cout<<"agrego un nuevo nodo"<<endl;
	printVec(copiasolucion);
	int largosol=copiasolucion.size();
	while(calculaValor(copiasolucion,distancias)<=presupuesto && distroide.size()>1){
		complemento_solucion.erase(complemento_solucion.begin()+pos);
		distroide = distanciascentroide(centro,intastruc(complemento_solucion,todoslosnodos));
		pos = 0;
		distancia_menor= distroide[0];
		for (int i = 1; i < distroide.size(); ++i){
			if(distancia_menor>distroide[i]){
				distancia_menor=distroide[i];
				pos=i;
		...
		it = copiasolucion.begin();
		int i=0;
		for(it;it!=copiasolucion.end();it++){
			it=copiasolucion.insert(it,complemento_solucion[pos]);
			if(calculaValor(copiasolucion,distancias)<=presupuesto){
				break;
			}else{
		...
		if(largosol<copiasolucion.size()){
			printVec(copiasolucion);
			largosol=copiasolucion.size();
			cout<<"Costo: "<<calculaValor(copiasolucion,distancias)<<" Largo sol: "<<copiasolucion.size()<<endl;
		}
...
//greedy(vector<int>a,vector<vector<double> >b,map<int,vector<int> >c,map<int,int>d+,ofstream &outputFile,double presupuesto,int nodos,vector<Pokeparada> auxpokeparada, char* str1){
vector<int> iteratedlocalsearch(vector<int>a,vector<vector<double> >b,map<int,vector<int> >c,map<int,int>d,ofstream &outputFile,double presupuesto,int nodos,vector<Pokeparada> auxpokeparada, char* str1,string &shuffl,string &shuflleaumentaprecio,int numcluster){
	...
	for (int i = 0; i < n; ++i){
		...
		s=agregarnodoalazar(s,b,presupuesto,auxpokeparada);
...
int main(int argc, char* argv[]){
	...
	while(!instanciaarchivo.eof()){
		...
		cout<<"pasa 2"<<endl;								// matriz de precios de viajar de un nodo a otro
		...
		for (int i = 0; i < 1; ++i){
			...
			//solucion=greedy(solucion,vecotorcosotos,mapaclusters,mapaverificacion,outputFile,atof(argv[2]),nodos,auxpokeparada,str1);
			/*printVec(solucion);
			solucion=greedy2(solucion,vecotorcosotos,complementosolucion,outputFile,atof(argv[2]));
			printVec(solucion);
			eliminarnodo(solucion,clusters,auxpokeparada,atof(argv[2]),vecotorcosotos);
			*/
